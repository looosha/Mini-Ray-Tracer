...
std::pair<bool, Vector3d> Dielectric::refract(const Vector3d &v, const Vector3d &normal) const {
    ...
    if (discriminant > 0.0) {
        //Vector3d refr_direction = refr_index_ratio * (v / v.norm()) - refr_index_ratio * (refr_norm * cos_initial) - refr_norm * sqrt(discriminant);
        //Vector3d refr_direction = refr_index_ratio * (v / v.norm()) - refr_norm * (refr_index_ratio * cos_initial + sqrt(discriminant));
        //Vector3d refr_direction = refr_index_ratio * (v / v.norm()) + refr_norm * (refr_index_ratio * cos_initial - sqrt(discriminant));
        /**
